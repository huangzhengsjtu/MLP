...
def sgd_optimization_mnist(learning_rate=0.13, n_epochs=1000,
    ...
    pp_test_model = theano.function(inputs=[index],
            outputs=classifier.pp_errors(y, 0.5, 1), on_unused_input='ignore',
            givens={
                x: test_set_x[index * batch_size: (index + 1) * batch_size],
                y: test_set_y[index * batch_size: (index + 1) * batch_size]})
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    pp_test_losses = [pp_test_model(i) for i in xrange(n_test_batches)]
                    pren = numpy.sum([te[1] for te in pp_test_losses])
                    ppn = numpy.sum([te[0] for te in pp_test_losses])
                    print 'ppn:',ppn, '  pren:',pren
